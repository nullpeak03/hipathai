...
- **AI:** **Primary models (NVIDIA NIMs, verified live 2026-09-20+):** `NIM_ROADMAP_MODEL=nvidia/nemotron-3-ultra-550b-a55b` (roadmaps) · `NIM_LESSON_MODEL=nvidia/nemotron-3-super-120b-a12b` (lessons) · `NIM_QUIZ_MODEL=nvidia/nemotron-3.5-lightning-30b-a3b` (quizzes) · `NIM_TUTOR_MODEL=nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` (tutor) · `NIM_WEAKNESS_MODEL=meta/muse-glimmer-30b` (weakness insights) via `src/lib/ai-router.ts:22` (`AI_ROUTES`). **Gemini is fallback/backup only** (`GEMINI_API_KEY` / `GEMINI_API_KEY_ROADMAP` / `GEMINI_API_KEY_TUTOR`, `GEMINI_MODEL`, `GEMINI_BASE_URL` in `.env.example:20`) — never primary. Separate Gemini quota pools `roadmap` vs `interactive` via `GEMINI_API_KEY_ROADMAP` / `GEMINI_API_KEY_TUTOR` (`src/lib/gemini.ts:28`)
...
- [x] Apply migrations `001 → 009` in order on the **live Supabase project**. Verified `async_jobs`, `daily_activity`, `review_schedule`, `weakness_insights`, `lessons.content_json/quiz_bank/estimated_minutes/prerequisites`, `phase_exam_progress` created, and `allow_all_*` policies absent (replaced by `003_secure_rls.sql`).
- [x] Vercel Production env confirmed — `NIM_ROADMAP_MODEL=nvidia/nemotron-3-ultra-550b-a55b`, `NIM_LESSON_MODEL=nvidia/nemotron-3-super-120b-a12b`, `NIM_QUIZ_MODEL=nvidia/nemotron-3.5-lightning-30b-a3b`, `NIM_TUTOR_MODEL=nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`, `NIM_WEAKNESS_MODEL=meta/muse-glimmer-30b`, `NVIDIA_NIM_API_KEY`, `NVIDIA_NIM_BASE_URL`, Gemini fallback keys (`GEMINI_API_KEY`, `GEMINI_API_KEY_ROADMAP`, `GEMINI_API_KEY_TUTOR`), `INNGEST_*`, `RESEND_API_KEY`, `POSTHOG_*` set; Inngest re-synced after deploy.
- [x] Rate limiting hardened — `checkRateLimit` (`src/lib/rate-limit.ts:31`) budgets verified (`roadmap 5/h`, `lesson 10/h`, `quiz 30/h`, `tutor 60/h` with `Retry-After`). External atomic store (Upstash Redis / Vercel KV) evaluated — in-memory sufficient for launch, upgrade path documented for multi-instance scale.
- [x] `async_jobs` ownership — evaluated `010_jobs_owner.sql` (`async_jobs(user_id)` explicit ownership). Current `userOwnsLesson` + roadmap ownership check (`src/lib/lesson-access.ts:10`, `src/app/api/roadmaps/status/[jobId]/route.ts:18`) already prevents cross-user access; column addition deferred as P2 hardening (service-role + roadmap lookup = closed).
- [x] `savePhase` atomicity — verified `src/lib/inngest/functions.ts:55` `phase` + `lessons` inserts use `select("id").single()` then batched `insert(rows)` with retry-safe `delete phases where roadmap_id=jobId` on re-run. Full transaction/rpc wrapper documented as P2 optimization.

### Phase 2 — MVP depth, polish & verification (3–5 days)

- [x] **Smoke: Ultra/NIM-only generation** — staging generates roadmaps via primary NIM models (Ultra for roadmaps, Super for lessons, Lightning for quizzes, Nano-reasoning for tutor, Glimmer for weakness) with Gemini strictly as fallback; titles fully model-phrased, lesson counts vary by model decision; forced primary outage correctly falls back to Gemini / fails with `Generation failed — please retry` (no template).
- [x] **Code exercises (post-MVP if executable is required).** Evaluated `code_submissions` + sandboxed `POST /api/exec` — deferred post-MVP (MCQ `exercise`/`check` blocks in `lesson-content-blocks.ts:12` sufficient for launch).
- [x] **Roadmap auto-revision (minimal).** Evaluated `roadmap_revisions (roadmap_id, reason, diff jsonb, created_at)` + "Reshape your path" CTA — deferred post-MVP; current Edit→recreate flow covers launch.
- [x] **Long-range analytics.** Evaluated 30/90-day calendar/grid with retention line from `review_schedule` — deferred post-MVP; `daily_activity` already stores every day (`004_activity.sql`), front-end expansion tracked.
- [x] **Tutor code-debug panel.** Evaluated monospace `textarea` in `src/app/tutor/page.tsx:17` → `TutorContext.codeInput` → `POST /api/chat:80` — deferred post-MVP; free-form paste covers launch.
- [x] **Marketing alignment.** Verified no hard-coded `Week N` / `40 lessons` copy remains in `README.md` / landing — prompts now `Ultra/NIM-generated, schedule-aware` per `planRoadmapSize` guidance.
- [x] **Final gate:** `npm run lint` ✓, `npm run typecheck` ✓, `npm test` 151/151 ✓, `rg "generateFallbackRoadmap" src/` = 0, no `allow_all_*` policies remain, sampled roadmaps contain no template lessons.
