...
export function buildRoadmapPrompt({ goal, level = "Beginner", time = "1hr/day", duration = "8 weeks", phases, lessons, why, styles }: RoadmapPromptInput): string {
  ...
  return `Generate a personalized roadmap for goal "${goal}". Level: ${level}, Time: ${time}, Duration: ${duration} (${phases} weeks).${motivation} Create EXACTLY ${phases} weekly phases (Week 1 to Week ${phases}) with ~${lessons} lessons total, paced so earlier weeks = foundations, middle = practice, final = synthesis. Phase and lesson titles must be concept phrases like "Introduction and Syntax": 2–4 words, Title Case, no numbering ("Phase X"/"Lesson X"), no goal repetition, unique across roadmap. Each lesson needs an "objective": a concise sentence (8-12 words). Each lesson is ~10 minutes, order so later lessons build on earlier (prerequisites).${styleSection(styles)} Return ONLY valid JSON: {title, description, phases:[{title, lessons:[{title, objective}]}]} No explanatory text, no markdown fences, no "Here's a thinking process:" prefix.`
}
...
export function buildOutlinePrompt({ goal, level = "Beginner", time = "1hr/day", duration = "8 weeks", why, styles, phases }: OutlinePromptInput): string {
  const motivation = why && why.trim().length > 0 ? ` Motivation: ${why.trim().slice(0, 200)}.` : ""
  return `Plan a personalized roadmap for goal "${goal}". Level: ${level}, Time: ${time}, Duration: ${duration} (${phases} weeks).${motivation} Create EXACTLY ${phases} weekly phases (Week 1 to Week ${phases}) in logical learning dependency order. Phase titles must be concept phrases like "Introduction and Syntax": 2–4 words, Title Case, no numbering, goal-specific and unique.${styleSection(styles)} Return ONLY valid JSON: {title, description, phases:[{title}]} No explanatory text, no markdown fences, no "Here's a thinking process:" prefix.`
}
...
export function buildPhasePrompt({ goal, level = "Beginner", time = "1hr/day", duration = "8 weeks", why, styles, phaseIndex, phaseCount, phaseTitle, lessonCount }: PhasePromptInput): string {
  const motivation = why && why.trim().length > 0 ? ` Motivation: ${why.trim().slice(0, 200)}.` : ""
  return `You are expanding Week ${phaseIndex} of ${phaseCount} (phase "${phaseTitle}") of a personalized roadmap for goal "${goal}". Level: ${level}, Time: ${time}, Duration: ${duration}.${motivation} Generate EXACTLY ${lessonCount} lessons for THIS week only, in learning order where each lesson builds on previous (prerequisites). Lesson titles must be concept phrases like "Introduction and Syntax": 2–4 words, Title Case, no numbering, goal-specific and unique. Each lesson needs an "objective": a concise sentence (8-12 words). Each lesson is ~10 minutes.${styleSection(styles)} Return ONLY valid JSON: {title, lessons:[{title, objective}]} No explanatory text, no markdown fences, no "Here's a thinking process:" prefix.`
}
