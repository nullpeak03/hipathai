...
export function Sidebar() {
  ...
  const inner = (
    <div className="flex flex-col h-full">
      <Link href="/dashboard" className="h-14 flex items-center gap-2 px-4 border-b hover:bg-gray-50">
      ...
      <nav className="flex-1 p-3 space-y-1">
        {nav.map(item => {
          ...
          return (
            <Link key={item.label} href={item.href} onClick={()=>setOpen(false)} className={cn("flex items-center gap-3 px-3 py-2 rounded-lg text-sm", isActive ? "bg-[#6C5BFF] text-white" : "text-zinc-600 hover:bg-gray-100")}>
              <item.icon className="w-4 h-4" /> {item.label}
            </Link>
          )
        })}
        <div className="pt-4">
          <button onClick={handleSignOut} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-zinc-600 hover:bg-gray-100 w-full text-left"><LogOut className="w-4 h-4" /> Sign out</button>
        </div>
      </nav>
    </div>
  )
