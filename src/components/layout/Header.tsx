    ...
    <header className="h-14 border-b bg-white flex items-center justify-between px-4 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative">
          <button onClick={handleBell} className="p-2 rounded-full hover:bg-gray-100 relative" title="Notifications" aria-label="Notifications">
          ...
          {bellOpen && (
            <div className="absolute right-0 mt-2 w-80 max-w-[90vw] rounded-xl border bg-white shadow-xl overflow-hidden z-30">
              <div className="flex justify-between items-center px-4 py-3 border-b">
                <span className="font-semibold text-sm">Notifications</span>
                <button onClick={()=> setBellOpen(false)} className="text-xs text-zinc-400 hover:text-zinc-600" aria-label="Close notifications">✕</button>
              </div>
              <div className="max-h-80 overflow-y-auto p-2">
                {reviews.length === 0 && weak.length === 0 && (
                  <p className="text-xs text-zinc-500 text-center py-6">All caught up. Reviews and weak areas will appear here.</p>
                )}
                {reviews.slice(0, 4).map((r) => (
                  <Link key={r.lessonId} href={`/roadmap/${r.roadmapId}/lesson/${r.lessonId}`} onClick={()=> setBellOpen(false)} className="block rounded-lg px-3 py-2 hover:bg-gray-50">
                    <div className="text-xs font-medium">📚 Review due: {r.title}</div>
                    <div className="text-[11px] text-zinc-500">{r.overdueDays > 0 ? `${r.overdueDays}d overdue` : "due today"}</div>
                  </Link>
                ))}
                {weak.slice(0, 4).map((w) => (
                  <Link key={w.topic} href={`/tutor?q=${encodeURIComponent(`Help me practice: ${w.topic}`)}`} onClick={()=> setBellOpen(false)} className="block rounded-lg px-3 py-2 hover:bg-gray-50">
                    <div className="text-xs font-medium">🎯 Weak area: {w.topic}</div>
                    <div className="text-[11px] text-zinc-500">failed {w.fail_count}× — practice with the tutor</div>
                  </Link>
                ))}
              </div>
              <Link href="/dashboard" onClick={()=> setBellOpen(false)} className="block text-center text-xs text-[#6C5BFF] py-2 border-t hover:bg-gray-50">View dashboard →</Link>
            </div>
          )}
        </div>
        <Link href="/settings" className="p-2 rounded-full hover:bg-gray-100" aria-label="Settings"><Settings className="w-4 h-4" /></Link>
