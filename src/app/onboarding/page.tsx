            ...
            <motion.div key={step} initial={{opacity:0, x:16}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-16}} transition={{duration:0.2}} className="flex-1">
              ...
              {current.id==="goal" ? (
                <div className="mt-6 space-y-3">
                  ...
                  {values.goal.trim().length>0 && values.goal.trim().length<3 && <p className="text-xs text-danger-fg">Goal must be at least 3 characters</p>}
                  <div className="flex flex-wrap gap-2">
                    {current.options?.map(s=> <button key={s.value} onClick={()=> setVal("goal", s.value)} className="text-xs border border-border px-3 py-1.5 rounded-full hover:bg-muted">{s.label}</button>)}
                  </div>
                </div>
              ...
              {!error && polling && pollStatus && <div className="mt-4 p-3 bg-info-bg border border-info-border rounded-lg text-sm text-info-fg">{pollStatus}</div>}
            </motion.div>
          </AnimatePresence>
...
export default function Onboarding() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-app flex items-center justify-center"><div className="animate-pulse h-8 w-48 bg-muted rounded"/></div>}>
      <OnboardingContent />
    </Suspense>
  )
}
